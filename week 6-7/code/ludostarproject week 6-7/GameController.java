...
public class GameController {
    ...
    public void startGame() {
        ...
        frame.showWindow();
        log("Welcome to Ludo Star. Red starts. Roll a 6 to leave the yard.");
        refresh();
    }
    ...
    public void rollDice() {
        if (gameOver || waitingForTokenChoice) {
            return;
        }
    ...
    public void newGame() {
        ...
        log("New game. Red starts.");
        status("Roll the dice to start your turn.");
        refresh();
    }
    ...
    private void moveToken(Token token) {
        ...
        if (token.isInYard()) {
            ...
            token.setStepCount(token.getStepCount() + roll);
            log("Token " + token.displayNumber() + " moved " + roll + " square(s).");
        }

        boolean captured = captureOpponents(player, token);
        String message = player.getName() + " moved token " + token.displayNumber() + ".";
        if (captured) {
        ...
        if (player.hasWon()) {
            ...
            SwingUtilities.invokeLater(() -> JOptionPane.showMessageDialog(
                    frame, player + " wins!", "Ludo Star", JOptionPane.INFORMATION_MESSAGE));
            return;
        }
