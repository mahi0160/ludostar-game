...
public class GameFrame extends JFrame {
    ...
    public void refresh() {
        ...
        Player[] activePlayers = controller.getActivePlayers();
            ...
            playersList.add(
                    playerRow(
                            ...
                            activePlayer.getName()
                                    + " ("
                                    + activePlayer.getColor().getDisplayName()
                                    + ")"));
        }
        ...
        for (int i = 0; i < tokenButtons.length; i++) {
            ...
            tokenButtons[i].setText(
                    "Token "
                            + token.displayNumber()
                            + " · "
                            + state);

            tokenButtons[i].setEnabled(
                    choosing
                            && controller.canMoveToken(
                                    token,
                                    roll));
        }
    ...
    private JPanel createRightPanel() {
        ...
        newGame.addActionListener(e -> {
            ...
            int numberOfPlayers = playerChoice + 2;

            PlayerColor[] selectedColors;

            if (numberOfPlayers == 4) {

                selectedColors = new PlayerColor[] {
                        PlayerColor.RED,
                        PlayerColor.GREEN,
                        PlayerColor.YELLOW,
                        PlayerColor.BLUE
                ...
                int[] selected = new int[numberOfPlayers];
                ...
                JPanel colorPanel = new JPanel(
                        new GridLayout(numberOfPlayers, 1, 5, 5));

                javax.swing.JCheckBox[] colorChecks = new javax.swing.JCheckBox[4];

                for (int i = 0; i < 4; i++) {
                    colorChecks[i] = new javax.swing.JCheckBox(colorNames[i]);

                    colorChecks[i].setFont(
                            new Font(
                                    "SansSerif",
                                    Font.BOLD,
                                    14));

                    colorPanel.add(colorChecks[i]);
                }
                ...
                selectedColors = new PlayerColor[numberOfPlayers];

                int index = 0;

                for (int i = 0; i < 4; i++) {
                    if (colorChecks[i].isSelected()) {
                        selectedColors[index++] = allColors[i];
                    }
                }
            }

            controller.setPlayerCount(numberOfPlayers);
            controller.setActiveColors(selectedColors);
            controller.newGame();
        });
